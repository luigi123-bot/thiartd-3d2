...
import { enviarEmailConfirmacionNodemailer } from "./emailConfirmacion";
...
async function enviarEmailConfirmacion(
  ...
  if (pedido) {
    const html = `
      ...
      <p>Tu pedido <b>#${pedidoId}</b> ha sido confirmado y está en proceso.</p>
      <p>Monto pagado: <b>$${transaction.amount_in_cents / 100}</b></p>
      <p>ID de pago: <b>${transaction.id}</b></p>
      <p>Pronto recibirás el número de seguimiento y más detalles.</p>
      <br>
      <p>Si tienes dudas, responde a este correo.</p>
    `;
    await enviarEmailConfirmacionNodemailer({
      to: transaction.customer_email,
      subject: `Confirmación de pedido #${pedidoId}`,
      html
    });
    console.log("✅ Email de confirmación enviado a:", transaction.customer_email);
  }
}
